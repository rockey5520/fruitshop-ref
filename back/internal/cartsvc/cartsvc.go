...
func (s *CartSvc) Get(w http.ResponseWriter, r *http.Request) {
	...
	cartFetched, err := cart.FindCartByCartID(s.db, cart_id)
	if err != nil {
...
func (s *CartSvc) CreateItem(w http.ResponseWriter, r *http.Request) {
	...
	createdCartItem, err := cartItem.SaveCartItem(s.db)
	if err != nil {
	...
	applySingleItemDiscounts(*createdCartItem, s.db)
	applyDualItemDiscounts(cartItem, s.db)

	// Recalculate Cart value , Discounts post adding/deleting item from cart and applying business rules
	calc.RecalcualtePayments(s.db, cartItem.CartID)
...
func (s *CartSvc) UpdateItem(w http.ResponseWriter, r *http.Request) {
	// Reading the request body from http request
	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		responses.ERROR(w, http.StatusUnprocessableEntity, err)
	...
	// Save the cart item entry into the database
	createdCartItem, err := cartItem.UpdateCartItem(s.db)
	if err != nil {
		formattedError := formaterror.FormatError(err.Error())
		responses.ERROR(w, http.StatusInternalServerError, formattedError)
		return
...
func (s *CartSvc) DeleteItem(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	cart_id := vars["cart_id"]
	...
	// Save the cart item entry into the database
	createdCartItem, err := cartItem.DeleteCartItem(s.db, cart_id, fruitname)
	if err != nil {
		formattedError := formaterror.FormatError(err.Error())
		responses.ERROR(w, http.StatusInternalServerError, formattedError)
		return
...
func (s *CartSvc) GetItems(w http.ResponseWriter, r *http.Request) {
	// Reading cart_id from request params
	vars := mux.Vars(r)
	cart_id := vars["cart_id"]
	cartItem := models.CartItem{}
	cartItems := cartItem.FindAllCartItems(s.db, cart_id)
